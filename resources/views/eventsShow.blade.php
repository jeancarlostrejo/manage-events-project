<x-main-layout>
    <div class="m-2 p-2 flex flex-col md:flex-row justify-between">
        <h3 class="mb-4 text-2xl font-bold text-indigo-700 dark:text-slate-300">{{ $event->title }}</h3>
        <div class="flex space-x-2 dark:text-slate-300">
            {{ __('From') }}:
    ...
    <div class="mb-16 flex flex-wrap">
        <div class="mb-6 w-full shrink-0 grow-0 basis-auto lg:mb-0 lg:w-6/12 lg:pr-6">
            <div class="flex flex-col">
                ...
                <div class="flex flex-col p-4">
                    <span class="text-indigo-600 dark:text-blue-400 font-semibold">{{ __('Host info') }}</span>
                    <div class="flex space-x-4 mt-6 bg-slate-200 dark:bg-gray-700 p-2 rounded-md">
                        <span>
                        ...
                        <div class="flex flex-col">
                            <span class="text-2xl dark:text-slate-300">{{ $event->user->name }}</span>
                            <span class="text-2xl dark:text-slate-300">{{ $event->user->email }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full shrink-0 grow-0 lg:w-6/12 lg:pl-6 bg-slate-50 dark:bg-gray-800 rounded-md p-2">
            <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">
                {{ __('Start Time') }}: <time>{{ $event->start_time->format('H:i A') }}</time>
            </p>
            <p>
            ...
            <div class="flex justify-end">
                <div class="flex flex-col">
                    <div class="mb-4 flex items-center text-sm font-medium">
                        ...
                        <div class="text-yellow-300">{{ $event->country->name }}, {{ $event->city->name }}</div>
                    </div>
                    ...
                    <div class="">
                        <form action="{{ route('events.comments.store', $event) }}" class="flex justify-between space-x-2" method="POST">
                            ...
                            <input type="text"
                                ...
                                name="content" id="content" placeholder="{{ __('Write a comment') }}">
                            <button type="submit"
